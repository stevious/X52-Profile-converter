...
class gui(tk.Tk):
    ...
    def __init__(self):
        ...
        # * window setup
        self.title("X52 -> X52PRO profile converter")
    ...
    def make_labels(self):
        self.file_label = tk.Label(self, text="X52 file:").grid(row=0,
                                                                  column=0,
                                                                  padx=10,
                                                                  sticky=tk.E)
    ...
    def open_file(self):
        path = askopenfilename(#initialdir=r"C:\Users",
                               title="Select X52 profile file (.pr0)",
    ...
    def convert(self):
        ...
        text = re.sub(r"\[controller\=1f73269.*?\]\]\]\]\]\]\]", "", text, 0, re.DOTALL)  # removing controller
        text = re.sub(r"\r\n\s*\r\n\s*", r"]\r\n", text)

        text = re.sub(r"\[controller\=e81d998b-c604-4d71-be97-35ca01439c7e group='Pro Flight'\r\n\s*\[member=c7719f41-f667-4514-bbb4-3f38c9e4d05a name=X52 shortname=X52\]",
                    r"[controller=1f732691-3bc6-41ec-a977-c5bf0b03a3dc group='Pro Flight'\r\n      [member=75bb6cc8-fb40-4be1-bf2b-4b10397a98a8 name=X52Pro shortname=X52Pro]",
                    text)

        text = re.sub(r"Fire",r"Launch", text)
        text = re.sub(r"'Pinkie Switch'",r"Pinkie", text)

        text = re.sub(r"\[button=0x00090007 name='Fire D']\r\n\s*\[button=0x00090008 name='Fire E']\r\n\s*\[button=0x00090006 name='Pinkie Switch']",
                    r"[button=0x00090006 name='Pinkie Switch']\r\n        [button=0x00090007 name='Fire D']\r\n        [button=0x00090008 name='Fire E']",
                    text)

        text = re.sub(r"0x00090018", r"0x0009001C", text)
        text = re.sub(r"0x00090014", r"0x00090018", text)
        text = re.sub(r"0x00090010", r"0x00090014", text)
        text = re.sub(r"0x0009001F", r"0x00090010", text)
        text = re.sub(r"0x0009001E", r"0x0009001F", text)
        text = re.sub(r"0x0009001A", r"0x0009001E", text)
        text = re.sub(r"0x00090020",r"0x00090013", text)

        text = re.sub(r"\[button=0x00090012 name='Wheel Scroll Down'\]\r\n\s*\[button=0x00090011 name='Wheel Scroll Up'\]",
                    r"[button=0x00090011 name='Wheel Scroll Up']\r\n        button=0x00090012 name='Wheel Scroll Down']",
                    text)

        text = re.sub(r"0x00010034", r"tempval", text)
        text = re.sub(r"0x00010033", r"0x00010034", text)
        text = re.sub(r"tempval", r"0x00010033", text)

        text = re.sub(r"0x00090022",r"0x00090011", text)
        text = re.sub(r"0x00090021",r"0x00090012", text)

        text = re.sub(r"'POV Hat 2' way=5",r"'POV 2' way=5",  text)

        text = re.sub(r"'POV Hat 3' way=5",r"'Throttle Hat' way=5", text)
        text = re.sub(r"\[slider=0x00090018 name=Mode",r"[slider name=Mode",  text)
        text = re.sub(r"0x00090019",r"0x0009001D", text)
        text = re.sub(r"'POV Hat 1'",r"'POV 1' way=5", text)

        # New
        text = re.sub(r"\[axis=0x00010035 name=Twist\]\r\n\s*\[axis=0x00010032 name=Throttle\]",
                    r"[axis=0x00010032 name=Throttle]\r\n        [axis=0x00010035 name=Twist]",
                    text)


        text = re.sub(r"Rudder",r"Twist", text)

        # New
        text = re.sub(r"\[axis=0x00010033 name='Rotary 2'\]\r\n\s*\[axis=0x00010034 name='Rotary 1'\]",
                    r"[axis=0x00010034 name='Rotary 1']\r\n        [axis=0x00010033 name='Rotary 2']",
                    text)

        text = re.sub(r"Ministick X Axis",r"Mouse X Axis", text)
        text = re.sub(r"Ministick Y Axis",r"Mouse Y Axis", text)

        text = re.sub(r"e8cb8c21-eb62-4268-8d2e-211c573bd12f fallback=e8cb8c21-eb62-4268-8d2e-211c573bd12f",
                r"cd957a00-26bf-4577-89ff-676166fe3b28 fallback=cd957a00-26bf-4577-89ff-676166fe3b28",
                    text)
